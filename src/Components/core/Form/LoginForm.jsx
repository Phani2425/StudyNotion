...
const LoginForm = () => {
  ...
  return (
    <form onSubmit={submitHandler} className='flex flex-col gap-6 mt-5'>
        ...
        <label htmlFor='pass' className='text-[0.875rem] text-richblack-5 leading-[1.375rem]'>Password <sup className='text-pink-200'>*</sup></label>
        <input  type={showPassword ? 'text' : 'password'} placeholder='Enter password' id='pass' name='password' value={FormData.password} onChange={changeHandler} required className='bg-richblack-800 p-[12px] rounded-md border-[1px] border-transparent border-b-richblack-25'></input>

        <button type='button' className='absolute top-[37px] right-3'>
            {
                showPassword? <FaRegEyeSlash fontSize={24} fill='#AFB2BF' onClick={()=>{setshowPassword(false);}} /> : <FaRegEye fontSize={24} fill='#AFB2BF' onClick={()=>setshowPassword(true)} />
            }
        </button>
